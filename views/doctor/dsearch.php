...
<body>
  ...
  <div class="form-box">
    <div class="form">
      <form class="search-form" action="/doctor/search" method="post">
  ...
  if (isset($_POST["search-submit"])) {
    ...
    if (!empty($_POST["ref"])) {
      ...
      if (!mysqli_stmt_prepare($stmt, $sql)) {
        header("Location: /doctor/search?error=sqlerror1");
      } else {
      ...
      if (empty($_POST["did"])) {
        header("Location: doctor/search?error=emptyfields");
        ...
        $stmt = mysqli_stmt_init($conn);
        if (!mysqli_stmt_prepare($stmt, $sql)) {
          header("Location: doctor/search?error=sqlerror");
        } else {
    ...
    if (!empty($_POST["ref"])) {
      ...
      if (empty($_POST["did"])) {
        ...
        $stmt = mysqli_stmt_init($conn);
        if (!mysqli_stmt_prepare($stmt, $sql)) {
          header("Location: /doctor/search?error=sqlerror2");
        } else {
          mysqli_stmt_bind_param($stmt, "sss", $did, $date, $time);
          mysqli_stmt_execute($stmt);
          $result = mysqli_stmt_get_result($stmt);
